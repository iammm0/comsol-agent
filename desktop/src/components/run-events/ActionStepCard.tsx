...
export function ActionStepCard({ event }: { event: RunEvent }) {
  ...
  if (type === "action_start") {
    ...
    const action = thought ? String(thought.action ?? JSON.stringify(thought)).slice(0, 80) : "";
    return (
      <div className="run-event-card run-event-card--action">
        ...
        <div className="run-event-card__main">
          <span className="run-event-card__title">执行</span>
          {action && <p className="run-event-card__detail">{action}</p>}
        </div>
      </div>
    );
  }
  ...
  if (type === "action_end") {
    const action = String(d.action ?? "完成");
    return (
  ...
  return (
    <div className={`run-event-card run-event-card--result run-event-card--result-${success ? "ok" : "fail"}`}>
      ...
      <div className="run-event-card__main">
        <span className="run-event-card__title">{message ?? status ?? "执行结果"}</span>
      </div>
    </div>
  );
}
