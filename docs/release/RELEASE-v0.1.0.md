# COMSOL Agent Desktop v0.1.0 — 首个版本发布说明
...
COMSOL Agent 首个正式桌面版发布。通过自然语言描述建模需求，自动生成可在 COMSOL Multiphysics 中打开的 `.mph` 模型文件。本版本提供 **仅 Windows** 安装包，安装包内已包含 **Java 11**，无需单独安装 Python 或 JDK。**最终构建的安装程序会自行引用打包进去的 Java 11 环境**，其余行为与测试环境一致（不依赖系统 JAVA_HOME、不自动下载）。
...
1. 打开 [GitHub Releases](https://github.com/iammm0/comsol-agent/releases)，找到 **COMSOL Agent Desktop v0.1.0**（或 tag `desktop-v0.1.0`）。
2. 在 **Assets** 中下载其一：
   - **`.exe`**（NSIS 安装程序）— 推荐
   - **`.msi`**（Windows Installer）
3. 运行安装程序，按提示完成安装。
4. 从开始菜单或桌面快捷方式启动 **COMSOL Agent**。
...
1. **build-bridge.ps1**：用 PyInstaller 将 Python 后端打成 `comsol-agent-bridge-<target>.exe`，放到 `desktop/src-tauri/binaries/`。
...
- **桌面端仅支持 Windows**：macOS 与 Linux 请使用 [从源码运行](https://github.com/iammm0/comsol-agent#方式二从源码运行含桌面应用) 或 Python API。
...
[MIT License](https://github.com/iammm0/comsol-agent/blob/main/LICENSE)
