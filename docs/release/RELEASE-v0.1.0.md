...
- **COMSOL Multiphysics**：本项目当前以 6.3 为目标版本
...
- **COMSOL 与 Java 配置**：在设置 → COMSOL 配置中通过 **「选择目录」** 在文件管理器里选择 COMSOL 6.3 的 `plugins` 目录及 Java 8/11 安装目录（可选，留空则使用安装包内嵌 Java 11），无需手动填写路径或改系统环境变量。
...
2. **COMSOL 与 Java**（设置 → COMSOL 配置）：点击 **「选择目录」** 在文件管理器中选取本机 COMSOL 6.3 的 `plugins` 目录；如需使用自备的 Java 8 或 11，可再选择对应 JDK 安装目录，留空则使用安装包内嵌的 Java 11。
